...
def update_chude():
    a = mongo.get_many_d(collection_name='News', filter_spec={'data:class_chude':[]}, filter_other = {'_id':1,'data:content': 1})
    ...
    for i in a[0]:
        ...
        doc['data:class_chude'] = class_title
        mongo.update_one('News', doc)
        #print(class_text_clustering)
        
    return True
def test():
    a = text_clustering('ngay ',class_name="class_chude")
