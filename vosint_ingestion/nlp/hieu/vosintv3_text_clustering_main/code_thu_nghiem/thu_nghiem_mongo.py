...
def check_tu_khoa_loai_tru(lst_tu_khoa_loai_tru,str):
     res = any(ele in str for ele in list(lst_tu_khoa_loai_tru['class']['tu_khoa_loai_tru'].strip("").split(",")))
     if res == True:
        return True
     elif res == False:
        return False
...
def check_tu_khoa_bat_buoc(lst_tu_khoa_bat_buoc,str):
    for j in range(1, len(lst_tu_khoa_bat_buoc['class'])-2 + 1):
        if all(x in str for x in list(lst_tu_khoa_bat_buoc['class']['tu_khoa_bat_buoc_{}'.format(j)].strip("").split(","))) == False :
