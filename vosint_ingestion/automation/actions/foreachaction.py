...
def get_action_class(name: str):
    action_cls = GotoAction if name == 'goto' else \
        GetUrlsAction if name == 'get_urls' else \
        GetNewsInfoAction if name == 'get_news_info' else \
        SelectAction if name == 'select' else \
        GetAttrAction if name == 'get_attr' else \
        ForeachAction if name == 'foreach' else \
        ClickAction if name =='click' else \
        TestAction if name =='test' else \
        LoginAction if name =='login' else None
    if action_cls is None:
        raise InternalError(ERROR_NOT_FOUND,
                            params={
                                'code': [f'{name.upper()}_ACTION'],
                                'msg': [f'{name} action']
                            })
...
class ForeachAction(BaseAction):
    ...
    def __run_actions(self, actions: list[dict], input_val, pipeline_id =None):
        ...
        for act in actions:
            ...
            action = get_action_class(act['name'])(self.driver,
                                                   self.storage,
                                                   **params)
