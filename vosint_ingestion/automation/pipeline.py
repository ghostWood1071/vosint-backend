...
class Pipeline:
    ...
    def __init__(self, driver: BaseDriver, storage: BaseStorage, actions: list[dict], pipeline_id = None):
        if not driver:
        ...
        if first_action_info['action_type'] != ActionType.INPUT:
            raise InternalError(ERROR_NOT_INPUT,
                                params={
                                    'code': ['FIRST_ACTION'],
                                    'msg': ['The first action']
                                })
    ...
    def run(self):
        ...
        for act in self.__common_actions:
            ...
            action = get_action_class(act['name'])(self.__driver,
                                                   self.__storage,
                                                   **params)
