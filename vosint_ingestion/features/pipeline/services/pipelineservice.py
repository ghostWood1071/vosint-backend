...
class PipelineService:
    ...
    def get_pipeline_by_id(self, id: str) -> PipelineForDetailsDto:
        ...
        pipeline = self.__mongo_repo.get_one(self.__collection_name,
                                             {'_id': id})
    ...
    def get_pipelines(self, text_search: str = None, enabled: bool = None, actived: bool = None, order: str = None, page_number: int = None, page_size: int = None) -> tuple[list[PipelineForListDto], int]:
        ...
        if isinstance(actived, bool):
            ...
            filter_spec['_id'] = {'$in': pipeline_ids} if actived \
                else {'$not': {'$in': pipeline_ids}}

        pipelines, total_docs = self.__get_pipelines(filter_spec,
                                                     order,
                                                     page_number,
                                                     page_size)
    ...
    def get_pipelines_for_run(self, ids: list[str] = None) -> list[PipelineForDetailsDto]:
        # Create filter conditions
        filter_spec = {
            'enabled': True
        }
    ...
    def __get_pipelines(self, filter_spec: dict = {}, order: str = None, page_number: int = None, page_size: int = None) -> tuple[list, int]:
        ...
        # Query from the database
        pipelines, total_docs = self.__mongo_repo.get_many(collection_name=self.__collection_name,
    ...
    def put_pipeline(self, pipeline: dict) -> str:
        ...
        if '_id' in norm_pipeline and norm_pipeline['_id']:
            existed = self.__mongo_repo.get_one(self.__collection_name,
                                                {'_id': norm_pipeline['_id']})
        ...
        if existed is None:
            ...
            # Insert
            pipeline_id = self.__mongo_repo.insert_one(collection_name=self.__collection_name,
                                                       doc=norm_pipeline)
        else:
            ...
            # Update
            update_success = self.__mongo_repo.update_one(collection_name=self.__collection_name,
                                                          doc=norm_pipeline)
    ...
    def clone_pipeline(self, from_id: str) -> str:
        ...
        if not pipeline:
            raise InternalError(ERROR_NOT_FOUND,
                                params={
                                    'code': ['PIPELINE'],
                                    'msg': [f'Pipeline with id: {from_id}']
                                })
