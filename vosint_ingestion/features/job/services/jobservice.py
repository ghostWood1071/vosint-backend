...
class JobService:
    ...
    def get_result_job(self,News,order_spec,pagination_spec):
        results = self.__mongo_repo.get_many_News(News,order_spec=order_spec,
    ...
    def get_log_history(self, id: str,order_spec,pagination_spec):
        results = self.__mongo_repo.get_many_his_log('his_log',{'pipeline_id':id},order_spec=order_spec,
                                                           pagination_spec=pagination_spec)

        return results

    def get_log_history_error_or_getnews(self, id: str,order_spec,pagination_spec):
        results = self.__mongo_repo.get_many_his_log('his_log',{"$and": [{'pipeline_id':id}, {"$or": [{"actione": 'GetNewsInfoAction'}, {"log": 'error'}]}]},order_spec=order_spec,
                                                           pagination_spec=pagination_spec)
    ...
    def start_job(self, id: str):
        ...
        pipeline_id = str(id)
        Scheduler.instance().add_job(id,
                                     start_job,
                                     pipeline_dto.cron_expr,
                                     args=[pipeline_dto.schema,id])
    ...
    def start_all_jobs(self, pipeline_ids: list[str] = None):
        ...
        enabled_pipeline_dtos = self.__pipeline_service \
            .get_pipelines_for_run(pipeline_ids)
        ...
        for pipeline_dto in enabled_pipeline_dtos:
            try:
                Scheduler.instance().add_job(pipeline_dto._id,
                                             func(pipeline_dto.schema),
                                             pipeline_dto.cron_expr)
