...
def start_all_jobs(pipeline_ids):#Danh sách Pipeline Id phân tách nhau bởi dấu , (VD: 636b5322243dd7a386d65cbc,636b695bda1ea6210d1b397f)
...
def get_result_job(News, order, page_number, page_size):
    return JSONResponse(job_controller.get_result_job( News, order, page_number, page_size))
...
def get_log_history(pipeline_id: str, order, page_number, page_size):
    return JSONResponse(job_controller.get_log_history(pipeline_id, order, page_number, page_size))
...
def get_log_history_error_or_getnews(pipeline_id: str, order, page_number, page_size):
    return JSONResponse(job_controller.get_log_history_error_or_getnews(pipeline_id, order, page_number, page_size))