...
class GlobalConfigs:
    ...
    def __init__(self):
        if GlobalConfigs.__instance is not None:
            raise InternalError(ERROR_SINGLETON_CLASS,
                                params={
        ...
        if cfg is None or cfg.get('api') is None or cfg.get('log') is None:
            raise InternalError(ERROR_MISSING_CONFIG,
                                params={
                                    'msg': ['API server or Log information']
                                })
        ...
        if self.API.get('port') is None or self.API.get('url') is None or self.API.get('public_dir') is None:
            raise InternalError(ERROR_MISSING_CONFIG,
                                params={
                                    'msg': ['API server information']
                                })
        if self.LOG.get('log_dir') is None or self.LOG.get('file_name') is None or self.LOG.get('when') is None or self.LOG.get('interval') is None or self.LOG.get('backup_count') is None:
            raise InternalError(ERROR_MISSING_CONFIG,
                                params={
                                    'msg': ['log information']
                                })
